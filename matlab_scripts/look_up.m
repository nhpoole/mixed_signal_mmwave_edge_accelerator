...
%
% L = min(data.length); (minimum length used in simulation)
...
%
% look_up(nfet, 'id', 'vgs', 0:0.1:1, 'vds', 0:0.1:1)
%
...
%
% diag(look_up(nfet, 'id', 'vgs', 0:0.1:1, 'vds', 0:0.1:1))

function output = look_up(data, outvar, varargin)

% default values for parameters
defaultL = min(data.length);
defaultVGS = data.vgs;
...
if mode == 3
    ...
    x = interpn(data.length, data.vsb, data.vds, data.vgs, xdata, ... 
                        par.L, par.vsb, par.vds, par.vgs);
    y = interpn(data.length, data.vsb, data.vds, data.vgs, ydata, ... 
                        par.L, par.vsb, par.vds, par.vgs);
            
    % permute so that VGS dimension always comes first
    x = squeeze(permute(x, [4 1 3 2]));
    y = squeeze(permute(y, [4 1 3 2]));                
        ...
        for j = 1:length(xdesired)
            ...
            elseif strcmp(numerator,'gm') && (strcmp(denominator,'cgg') || strcmp(denominator,'cgg'))
                ...
                if crossings > 1
                    ...
                    disp('*** Example: look_up(nfet, ''id_w'', ''gm_gds'', gm_gds, ''vgs'', nfet.vgs(10:end))');
                    disp(' ')
                    figure(1000)
    ...
    if length(data.vsb) > 1
        output = squeeze(interpn(data.length, data.vsb, data.vds, data.vgs, ydata, ... 
                            par.L, par.vsb, par.vds, par.vgs));
    else
        output = squeeze(interpn(data.length, data.vds, data.vgs, ydata, ... 
                            par.L, par.vds, par.vgs));
    end                
end
