function [ data ] = load_ngspice(rawfilename)
...
% Get the list of variable names. These are probably invalid as MATLAB
% variable names.
...
s = regexprep(s, '(', '_'); % Replace opening brackets with underscore.
s = regexprep(s, ')', ''); % Strip closing brackets.
s = regexprep(s, '[', '_'); % Replace opening brackets with underscore.
s = regexprep(s, ']', ''); % Strip closing brackets.
s = regexprep(s, '-', ''); % Strip dashes.
s = regexprep(s, '#', '_'); % Replace hash symbol with underscore.
s = regexprep(s, '@', ''); % Remove @ symbol.
s = regexprep(s, '\.', '_'); % Replace dot symbol with word.
s = regexprep(s, '\+', '_'); % Replace plus symbol with underscore.
...
for col=1:numPoints
	for var=1:numVariables
        ...
        if (string(data.flags) == "complex")
            % Read one complex value from the binary part of the file.
            [real_value, count1] = fread(fid, 1, 'double');
            [complex_value, count2] = fread(fid, 1, 'double');
            if (count1 == 0 || count2 == 0)
                warning('No data found for variable %d column %d.', var, col);
            else
                % Copy the value into the data matrix.
                data_matrix(var, col) = real_value + 1i*complex_value;
            end
        else     
...
while ~feof(fid)
    ...
    if ~isempty(byte)
        warning('All expected data have been read, but there still appears to be some data remaining in the file:\n\t%s', rawfilename);
    end
end
